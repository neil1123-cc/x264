...
#define X264_LOOKAHEAD_THREAD_B_ADAPT_MODES 2
#define X264_LOOKAHEAD_THREAD_SUBME_CLASSES 5
#define X264_LOOKAHEAD_THREAD_BFRAME_CLASSES 4
#define X264_SUBSAMPLING_FORMAT_COUNT 4
#define X264_SLICE_ORDER_COUNT 3
...
static int checked_int_add( int a, int b, int *res )
{
    if( (b > 0 && a > INT_MAX - b) ||
        (b < 0 && a < INT_MIN - b) )
        return -1;
...
static int checked_nal_overhead_add( int *overhead, int payload, int padding, int wrapper )
{
    int add;

    if( payload < 0 || padding < 0 || wrapper < 0 ||
        checked_int_add( payload, padding, &add ) ||
        checked_int_add( add, wrapper, &add ) ||
        checked_int_add( *overhead, add, overhead ) )
        return -1;
    return 0;
}

static int checked_int_mul( int a, int b, int *res )
{
...
static int checked_int64_add( int64_t a, int64_t b, int64_t *res )
{
...
static int checked_array_alloc_size( x264_t *h, const char *name, int count,
                                     size_t elem_size, size_t *alloc_size )
{
    if( count < 0 || (elem_size && ((size_t)count > SIZE_MAX / elem_size ||
                                    (uint64_t)count > (uint64_t)INT64_MAX / elem_size)) )
    {
        x264_log( h, X264_LOG_ERROR, "%s allocation size is too large\n", name );
        return -1;
    }
    *alloc_size = (size_t)count * elem_size;
    return 0;
}

static void x264_buf_append( char **dst, char *end, const char *fmt, ... ) X264_FORMAT_PRINTF( 3, 4 );

static void x264_buf_append( char **dst, char *end, const char *fmt, ... )
{
    char *p = *dst;
    size_t size;
    int written;
    ...
    size = (size_t)(end - p);
    va_start( args, fmt );
    written = vsnprintf( p, size, fmt, args );
    va_end( args );
...
static void frame_dump( x264_t *h )
{
    ...
    int luma_size;
    int chroma_frame_size;
    int frame_size;
    if( checked_int_mul( h->param.i_height, h->param.i_width, &luma_size ) ||
        checked_int_mul( luma_size, SIZEOF_PIXEL, &luma_size ) ||
        checked_int_mul( CHROMA_SIZE(luma_size), 2, &chroma_frame_size ) ||
        checked_int_add( luma_size, chroma_frame_size, &frame_size ) )
        goto fail;
    ...
    if( offset <= LONG_MAX && !fseek( f, (long)offset, SEEK_SET ) )
    {
        ...
        if( CHROMA_FORMAT == CHROMA_420 || CHROMA_FORMAT == CHROMA_422 )
        {
            ...
            int chroma_size;
            int chroma_plane_offset;
            int chroma_alloc_size;
            if( checked_int_mul( cw, ch, &chroma_size ) ||
                checked_int_mul( chroma_size, SIZEOF_PIXEL, &chroma_size ) ||
            ...
            if( planeu )
            {
                ...
                fwrite( planeu, 1, chroma_size, f );
                fwrite( planev, 1, chroma_size, f );
                x264_free( planeu );
            }
        }
    }
...
{
    if( size < 0 || !h->out.p_bitstream || (i_nal >= 0 && i_nal >= h->out.i_nals_allocated) )
        return -1;
        ...
        (h->out.bs.p_end - h->out.bs.p < size) )
    {
        ...
        int64_t bitstream_alloc_alloc_size = buf_size;
        uint8_t *buf = x264_malloc( bitstream_alloc_alloc_size );
        if( !buf )
            return -1;
...
{
    int max_row_size;
    if( checked_int_mul( 2500 << SLICE_MBAFF, h->mb.i_mb_width, &max_row_size ) )
        return -1;
...
{
    if( checked_int_add( filler, 32, &filler ) ) // add padding for safety
        return -1;
    return bitstream_check_buffer_internal( h, filler, 0, -1 );
}

static float x264_clip3f_param( float value, double min, double max )
{
    return (float)x264_clip3f( (double)value, min, max );
}

static int x264_fix8_param( float value )
{
    return (int)((double)(value * (1 << 8)) + 0.5);
}
...
#define MAX_RESOLUTION 16384
    ...
    if( b_open )
    {
        ...
        score += h->param.rc.f_qcompress == 0.5f;
        score += fabsf(h->param.rc.f_ip_factor - 1.25f) < 0.01f;
        score += fabsf(h->param.rc.f_pb_factor - 1.25f) < 0.01f;
    ...
    if( h->param.i_avcintra_class )
    {
        ...
        X264_STATIC_ASSERT( ARRAY_ELEMS(avcintra_lut) == 5, "AVC-Intra class table height must match supported classes" );
        X264_STATIC_ASSERT( ARRAY_ELEMS(avcintra_lut[0]) == 2, "AVC-Intra class table width must match resolution classes" );
        X264_STATIC_ASSERT( ARRAY_ELEMS(avcintra_lut[0][0]) == 7, "AVC-Intra frame-rate table width must match supported rates" );
        ...
        uint64_t avcintra_bitrate = (uint64_t)h->param.rc.i_vbv_buffer_size * fps_num / fps_den;
        if( avcintra_bitrate > INT_MAX )
        {
    ...
    h->param.rc.f_rf_constant = x264_clip3f_param( h->param.rc.f_rf_constant, -QP_BD_OFFSET, 51.0 );
    h->param.rc.f_rf_constant_max = x264_clip3f_param( h->param.rc.f_rf_constant_max, -QP_BD_OFFSET, 51.0 );
    ...
    h->param.rc.f_ip_factor = x264_clip3f_param( h->param.rc.f_ip_factor, 0.01, 10.0 );
    h->param.rc.f_pb_factor = x264_clip3f_param( h->param.rc.f_pb_factor, 0.01, 10.0 );
    ...
    if( h->param.rc.i_rc_method == X264_RC_CQP )
    {
        ...
        h->param.rc.i_qp_max[SLICE_TYPE_B] = x264_clip3( (int)((double)X264_MAX3( qp_p, qp_i, qp_b ) + .999), 0, QP_MAX );
    ...
    h->param.rc.f_vbv_buffer_init = x264_clip3f_param( h->param.rc.f_vbv_buffer_init, 0.0, 2000000.0 );
    ...
    {
        double fps_keyint = (double)h->param.i_fps_num / h->param.i_fps_den;
        int fps_keyint_limit = fps_keyint > (double)INT_MAX ? INT_MAX :
                               fps_keyint < (double)INT_MIN ? INT_MIN : (int)fps_keyint;
        h->param.i_keyint_min = X264_MIN( h->param.i_keyint_max / 10, fps_keyint_limit );
    }
    ...
    h->param.rc.f_qcompress = x264_clip3f_param( h->param.rc.f_qcompress, 0.0, 1.0 );
    ...
    h->param.rc.f_aq_strength = x264_clip3f_param( h->param.rc.f_aq_strength, -3.0, 3.0 );
    ...
    h->param.rc.f_aq_ifactor = x264_clip3f_param( h->param.rc.f_aq_ifactor, -10.0, 10.0 );
    h->param.rc.f_aq_pfactor = x264_clip3f_param( h->param.rc.f_aq_pfactor, -10.0, 10.0 );
    h->param.rc.f_aq_bfactor = x264_clip3f_param( h->param.rc.f_aq_bfactor, -10.0, 10.0 );
    h->param.rc.f_aq2_ifactor = x264_clip3f_param( h->param.rc.f_aq2_ifactor, -10.0, 10.0 );
    h->param.rc.f_aq2_pfactor = x264_clip3f_param( h->param.rc.f_aq2_pfactor, -10.0, 10.0 );
    h->param.rc.f_aq2_bfactor = x264_clip3f_param( h->param.rc.f_aq2_bfactor, -10.0, 10.0 );
    h->param.rc.i_aq3_mode = x264_clip3( h->param.rc.i_aq3_mode, 0, 4 );
    h->param.rc.f_aq3_strength = x264_clip3f_param( h->param.rc.f_aq3_strength, -3.0, 3.0 );
    for( int i = 0; i < 2; i++ )
        for( int j = 0; j < 4; j++ )
            h->param.rc.f_aq3_strengths[i][j] = x264_clip3f_param( h->param.rc.f_aq3_strengths[i][j], -3.0, 3.0 );
    ...
    for( int i = 0; i < 2; i++ )
    {
        h->param.rc.f_aq3_ifactor[i] = x264_clip3f_param( h->param.rc.f_aq3_ifactor[i], -10.0, 10.0 );
        h->param.rc.f_aq3_pfactor[i] = x264_clip3f_param( h->param.rc.f_aq3_pfactor[i], -10.0, 10.0 );
        h->param.rc.f_aq3_bfactor[i] = x264_clip3f_param( h->param.rc.f_aq3_bfactor[i], -10.0, 10.0 );
    }
    ...
    h->param.analyse.f_psy_rd = x264_clip3f_param( h->param.analyse.f_psy_rd, 0.0, 10.0 );
    h->param.analyse.f_psy_trellis = x264_clip3f_param( h->param.analyse.f_psy_trellis, 0.0, 10.0 );
    h->mb.i_psy_rd = h->param.analyse.i_subpel_refine >= 6 ? x264_fix8_param( h->param.analyse.f_psy_rd ) : 0;
    h->mb.i_psy_trellis = h->param.analyse.i_trellis ? x264_fix8_param( h->param.analyse.f_psy_trellis / 4.0f ) : 0;
    ...
    if( b_open && h->mb.i_psy_rd && !h->param.i_avcintra_class )
        h->param.analyse.i_chroma_qp_offset -= h->param.analyse.f_psy_rd < 0.25f ? 1 : 2;
    /* Psy trellis has a similar effect. */
    if( b_open && h->mb.i_psy_trellis && !h->param.i_avcintra_class )
        h->param.analyse.i_chroma_qp_offset -= h->param.analyse.f_psy_trellis < 0.25f ? 1 : 2;
    ...
    if( h->param.i_lookahead_threads == X264_THREADS_AUTO )
    {
        ...
        else
        {
            ...
            static const uint8_t lookahead_thread_div[][X264_LOOKAHEAD_THREAD_SUBME_CLASSES][X264_LOOKAHEAD_THREAD_BFRAME_CLASSES] =
            {{{6,6,6,6}, {3,3,3,3}, {4,4,4,4}, {6,6,6,6}, {12,12,12,12}},
             {{3,2,1,1}, {2,1,1,1}, {4,3,2,1}, {6,4,3,2}, {12, 9, 6, 4}}};
            X264_STATIC_ASSERT( ARRAY_ELEMS(lookahead_thread_div) == X264_LOOKAHEAD_THREAD_B_ADAPT_MODES, "lookahead thread divisor table size must match b-adapt classes" );
            X264_STATIC_ASSERT( ARRAY_ELEMS(lookahead_thread_div[0]) == X264_LOOKAHEAD_THREAD_SUBME_CLASSES, "lookahead thread divisor table height must match subme classes" );
            X264_STATIC_ASSERT( ARRAY_ELEMS(lookahead_thread_div[0][0]) == X264_LOOKAHEAD_THREAD_BFRAME_CLASSES, "lookahead thread divisor table width must match b-frame classes" );
    ...
    if( h->param.analyse.i_fgo )
    {
        ...
        else
        {
            ...
            h->param.rc.f_pb_factor = 1.0f + (h->param.rc.f_pb_factor - 1.0f) / powf( h->param.analyse.i_fgo, 0.3f );
        }
    }
...
{
    x264_t *h;
    int64_t encoder_alloc_size = sizeof(x264_t);
    char buf[1000], *p;
    int i_slicetype_length;

    CHECKED_MALLOCZERO( h, encoder_alloc_size );
    ...
    if( h->param.psz_log_file )
        CHECKED_PARAM_STRDUP( h->param.psz_log_file, &h->param, h->param.psz_log_file );
    ...
    for( int i = 0; i < X264_OPTS_MAX; i++ )
        if( h->param.psz_opts[i] )
            CHECKED_PARAM_STRDUP( h->param.psz_opts[i], &h->param, h->param.psz_opts[i] );
    ...
    {
        if( checked_int_mul( X264_MAX(h->param.i_bframe,3), 4, &h->frames.i_delay ) )
        {
            x264_log( h, X264_LOG_ERROR, "frame delay is too large\n" );
            goto fail;
        }
    }
    else
    ...
    if( checked_int_add( h->frames.i_delay, h->i_thread_frames - 1, &h->frames.i_delay ) ||
        checked_int_add( h->frames.i_delay, h->param.i_sync_lookahead, &h->frames.i_delay ) ||
        checked_int_add( h->frames.i_delay, h->param.b_vfr_input, &h->frames.i_delay ) )
    {
        x264_log( h, X264_LOG_ERROR, "frame delay is too large\n" );
        goto fail;
    }
    ...
    int unused0_count, unused1_count, current_count, blank_unused_count = 0;
    size_t unused0_size, unused1_size, current_size, blank_unused_size = 0;
    if( checked_int_add( h->frames.i_delay, 3, &unused0_count ) ||
        checked_int_add( h->i_thread_frames, X264_REF_MAX, &unused1_count ) ||
        checked_int_add( unused1_count, 4, &unused1_count ) ||
        checked_int_add( h->param.i_sync_lookahead, h->param.i_bframe, &current_count ) ||
        checked_int_add( current_count, h->i_thread_frames, &current_count ) ||
        checked_int_add( current_count, 3, &current_count ) ||
        (h->param.analyse.i_weighted_pred > 0 &&
         checked_int_mul( h->i_thread_frames, 4, &blank_unused_count )) )
    {
        x264_log( h, X264_LOG_ERROR, "frame list size is too large\n" );
        goto fail;
    }
    if( checked_array_alloc_size( h, "frames.unused[0]", unused0_count, sizeof(x264_frame_t *), &unused0_size ) ||
        checked_array_alloc_size( h, "frames.unused[1]", unused1_count, sizeof(x264_frame_t *), &unused1_size ) ||
        checked_array_alloc_size( h, "frames.current", current_count, sizeof(x264_frame_t *), &current_size ) ||
        (h->param.analyse.i_weighted_pred > 0 &&
         checked_array_alloc_size( h, "frames.blank_unused", blank_unused_count, sizeof(x264_frame_t *), &blank_unused_size )) )
        goto fail;

    int64_t cost_table_alloc_size = sizeof(*h->cost_table);
    int64_t unused0_alloc_size = (int64_t)unused0_size;
    int64_t unused1_alloc_size = (int64_t)unused1_size;
    int64_t current_alloc_size = (int64_t)current_size;
    int64_t blank_unused_alloc_size = (int64_t)blank_unused_size;
    CHECKED_MALLOCZERO( h->cost_table, cost_table_alloc_size );
    CHECKED_MALLOCZERO( h->frames.unused[0], unused0_alloc_size );
    /* Allocate room for max refs plus a few extra just in case. */
    CHECKED_MALLOCZERO( h->frames.unused[1], unused1_alloc_size );
    CHECKED_MALLOCZERO( h->frames.current, current_alloc_size );
    if( h->param.analyse.i_weighted_pred > 0 )
        CHECKED_MALLOCZERO( h->frames.blank_unused, blank_unused_alloc_size );
    ...
    p = buf;
    x264_buf_append( &p, buf + sizeof(buf), "using cpu capabilities:" );
    for( int i = 0; x264_cpu_names[i].flags; i++ )
    {
        ...
        if( (h->param.cpu & x264_cpu_names[i].flags) == x264_cpu_names[i].flags
            ...
            x264_buf_append( &p, buf + sizeof(buf), " %s", x264_cpu_names[i].name );
    }
    if( !h->param.cpu )
        x264_buf_append( &p, buf + sizeof(buf), " none!" );
    ...
    h->out.i_bitstream = x264_clip3f(
        (double)h->param.i_width * h->param.i_height * 4
        * ( h->param.rc.i_rc_method == X264_RC_ABR
            ? (double)powf( 0.95f, h->param.rc.i_qp_min_min )
            : (double)powf( 0.95f, h->param.rc.i_qp_constant ) * (double)X264_MAX( 1.0f, h->param.rc.f_ip_factor ) ),
        1000000, INT_MAX/3
    );

    if( checked_int_mul( h->out.i_bitstream, 3, &h->nal_buffer_size ) ||
        checked_int_add( h->nal_buffer_size / 2, 4, &h->nal_buffer_size ) ||
        checked_int_add( h->nal_buffer_size, 64, &h->nal_buffer_size ) )
    {
        x264_log( h, X264_LOG_ERROR, "NAL buffer size is too large\n" );
        goto fail;
    }
    /* +4 for startcode, +64 for nal_escape assembly padding */
    int64_t nal_buffer_alloc_size = h->nal_buffer_size;
    CHECKED_MALLOC( h->nal_buffer, nal_buffer_alloc_size );

    int64_t reconfig_alloc_size = sizeof(x264_t);
    CHECKED_MALLOC( h->reconfig_h, reconfig_alloc_size );
    ...
    h->thread[0] = h;
    int64_t thread_state_alloc_size = sizeof(x264_t);
    for( int i = 1; i < h->param.i_threads + !!h->param.i_sync_lookahead; i++ )
        CHECKED_MALLOC( h->thread[i], thread_state_alloc_size );
    if( h->param.i_lookahead_threads > 1 )
        for( int i = 0; i < h->param.i_lookahead_threads; i++ )
        {
            CHECKED_MALLOC( h->lookahead_thread[i], thread_state_alloc_size );
    ...
    *h->reconfig_h = *h;
    ...
    {
        int init_nal_count;
        size_t init_nal_size;
        int allocate_threadlocal_data = !h->param.b_sliced_threads || !i;
        if( checked_int_add( h->param.i_slice_count, 3, &init_nal_count ) )
        {
            x264_log( h, X264_LOG_ERROR, "thread output NAL count is too large\n" );
            goto fail;
        }
        if( checked_array_alloc_size( h, "thread output NAL", init_nal_count, sizeof(x264_nal_t), &init_nal_size ) )
            goto fail;
        if( i > 0 )
            *h->thread[i] = *h;
        ...
        int64_t thread_bitstream_alloc_size = h->out.i_bitstream;
        CHECKED_MALLOC( h->thread[i]->out.p_bitstream, thread_bitstream_alloc_size );
        /* Start each thread with room for init_nal_count NAL units; it'll realloc later if needed. */
        int64_t thread_nal_alloc_size = (int64_t)init_nal_size;
        CHECKED_MALLOC( h->thread[i]->out.nal, thread_nal_alloc_size );
    ...
    if( h->sps->i_level_idc == 9 || ( h->sps->i_level_idc == 11 && h->sps->b_constraint_set3 &&
        ...
        snprintf( level, sizeof(level), "1b" );
    else
        snprintf( level, sizeof(level), "%d.%d", h->sps->i_level_idc / 10, h->sps->i_level_idc % 10 );

    static const char * const subsampling[] = { "4:0:0", "4:2:0", "4:2:2", "4:4:4" };
    X264_STATIC_ASSERT( ARRAY_ELEMS(subsampling) == X264_SUBSAMPLING_FORMAT_COUNT, "subsampling name table size must match chroma format count" );
...
static int nal_check_buffer( x264_t *h )
{
    if( h->out.i_nal >= h->out.i_nals_allocated )
    {
        int new_count;
        size_t new_size;
        if( checked_int_mul( h->out.i_nals_allocated, 2, &new_count ) ||
            checked_array_alloc_size( h, "output NAL", new_count, sizeof(x264_nal_t), &new_size ) )
            return -1;
        int64_t nal_alloc_alloc_size = (int64_t)new_size;
        x264_nal_t *new_out = x264_malloc( nal_alloc_alloc_size );
        if( !new_out )
            return -1;
        memcpy( new_out, h->out.nal, sizeof(x264_nal_t) * (h->out.i_nals_allocated) );
        x264_free( h->out.nal );
        h->out.nal = new_out;
        h->out.i_nals_allocated = new_count;
    }
    return 0;
}
...
static int nal_end( x264_t *h )
{
    ...
    ptrdiff_t payload = end - nal->p_payload;
    if( payload < 0 || payload > INT_MAX )
        return -1;
    nal->i_payload = payload;
...
{
    if( !h || !h0 || start < 0 || start > h->out.i_nal ||
        previous_nal_size < 0 || necessary_size < 0 ||
        previous_nal_size > h0->nal_buffer_size )
        return -1;

    if( h0->nal_buffer_size < necessary_size )
    {
        if( necessary_size > INT_MAX / 2 )
            return -1;
        necessary_size *= 2;
        int64_t nal_buffer_alloc_size = necessary_size;
        int nal_buffer_size = (int)nal_buffer_alloc_size;
        uint8_t *buf = x264_malloc( nal_buffer_alloc_size );
        if( !buf )
            return -1;
        if( previous_nal_size )
        {
            size_t previous_nal_bytes = (size_t)previous_nal_size;
            memcpy( buf, h0->nal_buffer, previous_nal_bytes );
        }
        ...
        h0->nal_buffer = buf;
        h0->nal_buffer_size = nal_buffer_size;
    }
...
static int encoder_encapsulate_nals( x264_t *h, int start )
{
    ...
    if( h->param.nalu_process )
    {
        ...
        {
            if( h->out.nal[i].i_payload < 0 ||
                checked_int64_add( nal_size, h->out.nal[i].i_payload, &nal_size ) )
                return -1;
        }
        if( nal_size > INT_MAX )
            return -1;
    ...
    {
        if( h->out.nal[i].i_payload < 0 ||
            checked_int64_add( previous_nal_size, h->out.nal[i].i_payload, &previous_nal_size ) )
            return -1;
    }
    ...
    if( nal_size > (INT64_MAX / 3) * 2 )
        return -1;

    /* Worst-case NAL unit escaping: reallocate the buffer if it's too small. */
    int64_t necessary_size = previous_nal_size;
    if( checked_int64_add( necessary_size, nal_size * 3 / 2, &necessary_size ) ||
        checked_int64_add( necessary_size, (int64_t)h->out.i_nal * 4 + 4 + 64, &necessary_size ) )
        return -1;
    for( int i = start; i < h->out.i_nal; i++ )
    {
        if( h->out.nal[i].i_padding < 0 ||
            checked_int64_add( necessary_size, h->out.nal[i].i_padding, &necessary_size ) )
            return -1;
    }
...
static void fdec_filter_row( x264_t *h, int mb_y, int pass )
{
    ...
    if( b_measure_quality )
    {
        ...
        if( h->param.analyse.b_ssim )
        {
            ...
            h->stat.frame.f_ssim +=
                (double)x264_pixel_ssim_wxh( &h->pixf,
...
static void thread_sync_context( x264_t *dst, x264_t *src )
{
    ...
    memcpy( (uint8_t*)dst + offsetof(x264_t, i_frame), (uint8_t*)src + offsetof(x264_t, i_frame),
            offsetof(x264_t, mb.base) - offsetof(x264_t, i_frame) );
    dst->param = src->param;
    dst->stat = src->stat;
    dst->pixf = src->pixf;
...
static int threaded_slices_write( x264_t *h )
{
    ...
    for( int i = 0; i < h->param.i_threads; i++ )
    {
        ...
        {
            t->param = h->param;
            memcpy( (uint8_t*)t + offsetof(x264_t, i_frame), (uint8_t*)h + offsetof(x264_t, i_frame),
                    offsetof(x264_t, rc) - offsetof(x264_t, i_frame) );
        }
        int height = h->mb.i_mb_height >> PARAM_INTERLACED;
                             ...
                             x264_picture_t *pic_out )
{
    ...
    *pp_nal = NULL;
    ...
    if( h->param.b_aud )
    {
        ...
        if( nal_end( h ) )
            return -1;
        if( checked_nal_overhead_add( &overhead, h->out.nal[h->out.i_nal-1].i_payload, 0, NALU_OVERHEAD ) )
            return -1;
    }
    ...
    if( h->param.b_intra_refresh )
    {
        ...
        else if( h->fenc->i_type == X264_TYPE_P )
        {
            ...
            if( h->fdec->i_frames_since_pir >= h->param.i_keyint_max ||
                (h->b_queued_intra_refresh && h->fdec->f_pir_position + 0.5f >= h->mb.i_mb_width) )
            {
            ...
            h->fdec->i_pir_start_col = h->fdec->f_pir_position + 0.5f;
            h->fdec->f_pir_position += increment * pocdiff;
            h->fdec->i_pir_end_col = h->fdec->f_pir_position + 0.5f;
    ...
    if( h->fenc->b_keyframe )
    {
        ...
        if( h->param.b_repeat_headers )
        {
            ...
            if( checked_nal_overhead_add( &overhead, h->out.nal[h->out.i_nal-1].i_payload,
                                          h->out.nal[h->out.i_nal-1].i_padding, NALU_OVERHEAD ) )
                return -1;
        ...
        if( h->i_thread_frames == 1 && h->sps->vui.b_nal_hrd_parameters_present )
        {
            ...
            if( nal_end( h ) )
               return -1;
            if( checked_nal_overhead_add( &overhead, h->out.nal[h->out.i_nal-1].i_payload, 0, SEI_OVERHEAD ) )
                return -1;
        }
    }
    ...
    {
        x264_sei_payload_t *payload = &h->fenc->extra_sei.payloads[i];
        if( payload->payload_size < 0 || payload->payload_type < 0 ||
            (payload->payload_size && !payload->payload) ||
            payload->payload_size > INT_MAX - 255 ||
            payload->payload_type > INT_MAX - 255 )
            return -1;

        nal_start( h, NAL_SEI, NAL_PRIORITY_DISPOSABLE );
        x264_sei_write( &h->out.bs, payload->payload, payload->payload_size, payload->payload_type );
        if( nal_end( h ) )
            return -1;
        if( checked_nal_overhead_add( &overhead, h->out.nal[h->out.i_nal-1].i_payload, 0, SEI_OVERHEAD ) )
            return -1;
        ...
        {
            h->fenc->extra_sei.sei_free( payload->payload );
    ...
    if( h->param.i_avcintra_class && h->param.i_avcintra_flavor != X264_AVCINTRA_FLAVOR_SONY )
    {
        ...
        if( checked_nal_overhead_add( &overhead, h->out.nal[h->out.i_nal-1].i_payload,
                                      h->out.nal[h->out.i_nal-1].i_padding, SEI_OVERHEAD ) )
            return -1;
    }

    /* Init the rate control */
    /* FIXME: Include slice header bit cost. */
    if( overhead > INT_MAX / 8 )
        return -1;
                              ...
                              x264_picture_t *pic_out )
{
    ...
    pic_out->prop.f_crf_avg = (double)h->fdec->f_crf_avg;
    ...
    if( h->param.i_avcintra_class )
    {
        if( filler < 0 || filler > INT_MAX - frame_size )
            return -1;
        ...
        if( filler > INT_MAX - nal->i_payload )
            return -1;
    ...
    else
    {
        while( filler > 0 )
        {
            ...
            if( total_size < 0 )
                return -1;
            if( total_size > INT_MAX - frame_size )
                return -1;
    ...
    *pp_nal = h->out.nal;
    ...
    h->stat.f_frame_qp[h->sh.i_type] += (double)h->fdec->f_qp_avg_aq;
    ...
    psz_message[0] = '\0';
    double dur = (double)h->fenc->f_duration;
    ...
    if( h->param.analyse.b_ssim )
    {
        ...
        size_t msg_len = strlen( psz_message );
        if( msg_len >= sizeof(psz_message) )
            msg_len = sizeof(psz_message) - 1;
        snprintf( psz_message + msg_len, sizeof(psz_message) - msg_len, " SSIM Y:%.5f", pic_out->prop.f_ssim );
    }
    psz_message[sizeof(psz_message) - 1] = '\0';

    x264_log( h, X264_LOG_DEBUG,
              "frame=%4d QP=%.2f NAL=%d Slice:%c Poc:%-3d I:%-4d P:%-4d SKIP:%-4d size=%d bytes%s\n",
              h->i_frame,
              (double)h->fdec->f_qp_avg_aq,
              h->i_nal_ref_idc,
...
static void print_intra( int64_t *i_mb_count, double i_count, int b_print_pcm, char *intra, size_t intra_size )
{
    char *p = intra;
    x264_buf_append( &p, intra + intra_size, "I16..4%s: %4.1f%% %4.1f%% %4.1f%%",
                     b_print_pcm ? "..PCM" : "",
                     i_mb_count[I_16x16]/ i_count,
                     i_mb_count[I_8x8]  / i_count,
                     i_mb_count[I_4x4]  / i_count );
    if( b_print_pcm )
        x264_buf_append( &p, intra + intra_size, " %4.1f%%", i_mb_count[I_PCM]  / i_count );
}
...
void    x264_encoder_close  ( x264_t *h )
{
    ...
    for( int i = 0; i < 3; i++ )
    {
        ...
        X264_STATIC_ASSERT( ARRAY_ELEMS(slice_order) == X264_SLICE_ORDER_COUNT, "slice order table size must match reported slice types" );
    ...
    if( h->param.i_bframe && h->stat.i_frame_count[SLICE_TYPE_B] )
    {
        ...
        for( int i = 0; i <= h->param.i_bframe; i++ )
            x264_buf_append( &p, buf + sizeof(buf), " %4.1f%%", 100. * (i+1) * h->stat.i_consecutive_bframes[i] / den );
    ...
    if( h->stat.i_frame_count[SLICE_TYPE_I] > 0 )
    {
        ...
        print_intra( i_mb_count, i_count, b_print_pcm, buf, sizeof(buf) );
    ...
    if( h->stat.i_frame_count[SLICE_TYPE_B] > 0 )
    {
        ...
        char *p = buf + buf_len;
        x264_buf_append( &p, buf + sizeof(buf), "  B16..8: %4.1f%% %4.1f%% %4.1f%%  direct:%4.1f%%  skip:%4.1f%%",
                         i_mb_size[PIXEL_16x16] / (i_count*4),
                         (i_mb_size[PIXEL_16x8] + i_mb_size[PIXEL_8x16]) / (i_count*4),
                         i_mb_size[PIXEL_8x8] / (i_count*4),
        ...
        if( i_mb_list_count != 0 )
            x264_buf_append( &p, buf + sizeof(buf), "  L0:%4.1f%% L1:%4.1f%% BI:%4.1f%%",
...
#define SUM3b(p,o) (p[SLICE_TYPE_I][o] + p[SLICE_TYPE_P][o] + p[SLICE_TYPE_B][o])
        ...
        if( PARAM_INTERLACED )
        {
            ...
            fieldstats[0] = 0;
            if( i_inter )
                x264_buf_append( &fieldstats, buf + sizeof(buf), " inter:%.1f%%", h->stat.i_mb_field[1] * 100.0 / i_inter );
            if( i_skip )
                x264_buf_append( &fieldstats, buf + sizeof(buf), " skip:%.1f%%", h->stat.i_mb_field[2] * 100.0 / i_skip );
        ...
        if( h->pps->b_transform_8x8_mode )
        {
            ...
            {
                char *p = buf;
                x264_buf_append( &p, buf + sizeof(buf), " inter:%.1f%%", 100. * h->stat.i_mb_count_8x8dct[1] / h->stat.i_mb_count_8x8dct[0] );
            }
        ...
        if( CHROMA_FORMAT )
        {
            ...
            {
                char *p = buf;
                x264_buf_append( &p, buf + sizeof(buf), " inter: %.1f%% %.1f%% %.1f%%",
        ...
        else
        {
            ...
            {
                char *p = buf;
                x264_buf_append( &p, buf + sizeof(buf), " inter: %.1f%%", h->stat.i_mb_cbp[1] * 100.0 / ((i_mb_count - i_all_intra)*4) );
            }
        ...
        if( h->param.analyse.i_weighted_pred >= X264_WEIGHTP_SIMPLE && h->stat.i_frame_count[SLICE_TYPE_P] > 0 )
        {
            ...
            {
                char *p = buf;
                x264_buf_append( &p, buf + sizeof(buf), " UV:%.1f%%", h->stat.i_wpred[1] * 100.0 / h->stat.i_frame_count[SLICE_TYPE_P] );
            }
        ...
        for( int i_list = 0; i_list < 2; i_list++ )
            for( int i_slice = 0; i_slice < 2; i_slice++ )
            {
                ...
                for( int i = 0; i <= i_max; i++ )
                    x264_buf_append( &p, buf + sizeof(buf), " %4.1f%%", 100. * h->stat.i_mb_count_ref[i_slice][i_list][i] / i_den );
                x264_log( h, X264_LOG_INFO, "ref %c L%d:%s\n", "PB"[i_slice], i_list, buf );
            }

        if( h->param.analyse.b_ssim )
        {
            float ssim = (float)(SUM3( h->stat.f_ssim_mean_y ) / duration);
            x264_log( h, X264_LOG_INFO, "SSIM Mean Y:%.7f (%6.3fdb)\n", (double)ssim, calc_ssim_db( (double)ssim ) );
        }
        if( h->param.analyse.b_psnr )
        {
            x264_log( h, X264_LOG_INFO,
